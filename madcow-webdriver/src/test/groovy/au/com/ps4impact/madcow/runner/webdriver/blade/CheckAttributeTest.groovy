 ...
 * @author Gavin Bunney
 */
class CheckAttributeTest extends AbstractBladeTestCase {

	CheckAttribute checkAttribute = new CheckAttribute()

    protected verifyCheckAttribute(GrassBlade blade, boolean shouldPass) {
    ...
    void testCheckAttributeByHtmlId() {
        // defaults to html id
        GrassBlade blade = new GrassBlade('aLinkId.checkAttribute = [name:"aLinkName"]', testCase.grassParser);
        verifyCheckAttribute(blade, true);

        // explicit id
        MadcowMappings.addMapping(testCase, 'mapping', ['id': 'aLinkId']);
        blade = new GrassBlade('mapping.checkAttribute = [name:"aLinkName"]', testCase.grassParser);
        verifyCheckAttribute(blade, true);
    }
	...
	void testCheckAttributeIncorrect() {
		// defaults to html id
		GrassBlade blade = new GrassBlade('aLinkId.checkAttribute = [name:"wrongName"]', testCase.grassParser);

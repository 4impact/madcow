...
/**
 * Checks that an element has less than or equals the specified number of rows
 *
 * @author Paul Bevis
 */
class LessThanOrEquals extends CountRows {
    ...
    void comparison(WebDriverStepRunner stepRunner, MadcowStep step) {
        ...
        if (rows <= rowNumberToCompare) {
            ...
            step.result = MadcowStepResult.FAIL("Number of rows found (${rows}) is not less than or equals the supplied value ($rowNumberToCompare)");
        }
    }
}

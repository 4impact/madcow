...
/**
 * Checks that an element has greater than the specified number of rows
 *
 * @author Paul Bevis
 */
class GreaterThan extends CountRows {
    ...
    void comparison(WebDriverStepRunner stepRunner, MadcowStep step) {
        ...
        if (rows > rowNumberToCompare) {
            ...
            step.result = MadcowStepResult.FAIL("Number of rows found (${rows}) is not greater than the supplied value ($rowNumberToCompare)");
        }
    }
}

...
/**
 * Checks that an element has less than the specified number of rows
 *
 * @author Paul Bevis
 */
class LessThan extends CountRows {
    ...
    void comparison(WebDriverStepRunner stepRunner, MadcowStep step) {
        ...
        if (rows < rowNumberToCompare) {
            ...
            step.result = MadcowStepResult.FAIL("Number of rows found (${rows}) is not less than the supplied value ($rowNumberToCompare)");
        }
    }
}

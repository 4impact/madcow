...
/**
 * Checks that an element has the specified number of rows
 *
 * @author Paul Bevis
 */
class Equals extends CountRows {
    ...
    void comparison(WebDriverStepRunner stepRunner, MadcowStep step) {
        ...
        if (rows == rowNumberToCompare) {
            ...
            step.result = MadcowStepResult.FAIL("Number of rows found (${rows}) does not equal supplied value ($rowNumberToCompare)");
        }
    }
}

...
/**
 * This class is SelectFrame which allows you to switch into a different frame
 *
 * @author Tom Romano
 */
class SelectFrame extends WebDriverBladeRunner {
    ...
    public void execute(WebDriverStepRunner stepRunner, MadcowStep step) {
        ...
        try{
            ...
            if (element!=null){
                stepRunner.driver.switchTo().frame(element)
                step.result = MadcowStepResult.PASS()
            }else{
                step.result = MadcowStepResult.FAIL("No match could be found for the expected element")
            }
        }catch (Exception e){
            step.result = MadcowStepResult.FAIL("Could not switch to the provided frame")
        }
